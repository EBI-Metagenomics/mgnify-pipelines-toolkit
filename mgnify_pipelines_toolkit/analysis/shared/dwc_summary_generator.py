...
def generate_dwcready_summaries(
    ...
    analyses to create standardized, Darwin Core-compliant summary files. It handles both SILVA
    and PR2 database results, combining taxonomy assignments with ENA metadata.
    ...
    Args:
        ...
        analyses_dir (Path): Input directory containing all individual analyses subdirectories
                            ...
                            and ASV results.
        output_prefix (str): Prefix to be used for the output summary files. The function will
                            generate multiple files with this prefix, one for each combination
                            of analysis method (DADA2/closed-reference) and database (SILVA/PR2).

    Returns:
        None: Writes output CSV files with the following naming pattern:
              - {output_prefix}_DADA2_SILVA_dwcready.csv
              - {output_prefix}_DADA2_PR2_dwcready.csv
    ...
    for db in asv_dbs:
        ...
        final_df.to_csv(
            f"{output_prefix}_DADA2_{db}_dwcready.csv", index=False, na_rep="NA"
        )
