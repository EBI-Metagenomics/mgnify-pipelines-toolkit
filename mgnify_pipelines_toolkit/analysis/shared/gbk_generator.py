...
def _open_fasta_text(path: str) -> TextIO:
    p = Path(path)
    if p.suffix == ".gz":
...
def read_fasta_records_dict(fasta_path: str) -> dict[str, SeqRecord]:
    ...
    with _open_fasta_text(fasta_path) as handle:
...
def read_protein_seqs_dict(
    ...
    with _open_fasta_text(proteins_path) as handle:
        for rec in SeqIO.parse(handle, "fasta"):
...
def read_prodigal_faa_with_validation(
    faa_path: str, drop_terminal_stop: bool, strict_headers: bool
    ...
    with _open_fasta_text(faa_path) as handle:
        for rec in SeqIO.parse(handle, "fasta"):
            total += 1
...
def build_records_from_gff_and_faa(
    *,
    ...
    with _open_fasta_text(gff_path) as handle:
        for rec in GFF.parse(handle, base_dict=contigs):
            ...
            features_out: list[SeqFeature] = [
                make_source_feature(
                    rec, note="generated by gbk_generator.py (GFF+FAA mode)"
                )
            ]
...
def build_records_from_prodigal_faa(
    *,
    ...
    for c in calls:
        if c.contig_id not in contigs:
            if skip_missing_contigs:
                continue
            raise ValueError(
                f"Prodigal FAA calls reference contig not found in contigs FASTA: {c.contig_id}"
            )
    ...
    for contig_id, base_rec in contigs.items():
        ...
        features_out: list[SeqFeature] = [
            make_source_feature(
                rec, note="generated by gbk_generator.py (Prodigal mode)"
            )
        ]
        ...
        for cds in feats:
            ...
            qualifiers: dict[str, list[str]] = {
                ...
                "product": [product_val],
                "note": [
                    f"gene_caller={gene_caller_version}"
                ],  # provenance in Prodigal mode
            }
            if gene_from_locus_tag:
                qualifiers["gene"] = [locus_tag_val]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--gff",
        required=False,
        help="CDS GFF3 (optionally .gz). Uses BCBio.GFF. Required for default mode.",
    )
    p.add_argument(
        "--faa",
        required=False,
        help="Proteins FASTA for translations (optionally .gz). Required for default mode.",
    )

    # Prodigal mode arguments
    p.add_argument(
        "--prodigal-headers-faa",
        required=False,
        help="Prodigal proteins FAA with coords in headers (optionally .gz). Enables Prodigal mode.",
    )
    ...
    p.add_argument(
        "--gene-caller-version",
        default="unknown",
        help="Gene caller tool and version for provenance notes",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="In Prodigal mode, drop CDS calls whose contig is missing",
    )
    p.add_argument(
        "--require-translation",
        action="store_true",
        help="In Prodigal mode, fail if translation cannot be produced",
    )
    p.add_argument(
...
def main() -> None:
    ...
    if prodigal_mode and default_mode:
        raise ValueError(
            "Cannot use both default mode (--gff + --faa) and Prodigal mode (--prodigal-headers-faa). Choose one mode."
        )
    ...
    if prodigal_mode:
        ...
        if _file_is_readable(args.gff) or _file_is_readable(args.faa):
            raise ValueError(
                "Prodigal mode: cannot provide --gff or --faa when using --prodigal-headers-faa"
            )
        ...
        if args.prodigal_headers_faa:
            raise ValueError(
                "Default mode: cannot provide --prodigal-headers-faa when using --gff + --faa"
            )
