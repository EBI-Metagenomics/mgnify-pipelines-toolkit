...
def build_records_from_gff(
    *,
    ...
    if _file_is_readable(proteins_path):
        proteins = read_protein_seqs_dict(proteins_path, drop_terminal_stop=drop_terminal_stop)
    ...
    with fileinput.hook_compressed(gff_path, "rt", encoding="utf-8", errors="ignore") as handle:
        for rec in GFF.parse(handle, base_dict=contigs):
            ...
            features_out: list[SeqFeature] = [make_source_feature(rec, note="generated by gbk_generator.py (GFF mode)")]
...
def build_records_from_prodigal_faa(
    *,
    ...
    if _file_is_readable(proteins_path):
        proteins_by_id = read_protein_seqs_dict(proteins_path, drop_terminal_stop=drop_terminal_stop)
        logger.info("Loaded proteins (explicit): %d", len(proteins_by_id))
    else:
        proteins_by_id = read_protein_seqs_dict(faa_path, drop_terminal_stop=drop_terminal_stop)
    ...
    for c in calls:
        if c.contig_id not in contigs:
            if skip_missing_contigs:
                continue
            raise ValueError(f"FAA calls reference contig not found in contigs FASTA: {c.contig_id}")
    ...
    for contig_id, base_rec in contigs.items():
        ...
        features_out: list[SeqFeature] = [make_source_feature(rec, note="generated by gbk_generator.py (FAA/Prodigal mode)")]
        ...
        for cds in feats:
            ...
            translation: str | None = proteins_by_id.get(cds.gene_id) if proteins_by_id else None
            if not translation:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gff", required=False, help="CDS GFF3 (optionally .gz). Uses BCBio.GFF.")
    p.add_argument("--faa", required=False, help="Prodigal proteins FAA with coords in headers (optionally .gz)")
    p.add_argument("--proteins", required=False, help="Proteins FASTA for translations (optionally .gz)")
