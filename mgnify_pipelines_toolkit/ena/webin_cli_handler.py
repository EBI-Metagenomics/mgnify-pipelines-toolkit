...
def check_manifest(manifest_file, workdir):
    ...
    if workdir and not fasta_location.startswith("/"):
        ...
        print(f"New manifest {manifest_for_submission} was created")
    return fasta_location, assembly_name, manifest_for_submission


def run_webin_cli(manifest, context, webin, password, mode, test):
    cmd = ["ena-webin-cli", f"-context={context}", f"-manifest={manifest}", f"-userName={webin}", f"-password='{password}'", f"-{mode}"]
    if test:
        cmd.append("-test")
    cmd_str = " ".join(cmd)

    for attempt in range(1, RETRIES + 1):
        print(f"🚀 Running ena-webin-cli (attempt {attempt}/{RETRIES})...")
        ...
        print(f"❌ ena-webin-cli failed with code {result.returncode}")
...
def check_submission_status_live(report_text):
    ...
    if "submission has been completed successfully" in report_text:
        print(f"Submission for that object was done first time on MAIN server")
        return True, submission_exists
    elif "object being added already exists in the submission account with accession" in report_text:
        print(f"Submission for that object has already exist on MAIN server")
        submission_exists = True
        return True, submission_exists
    else:
        print(f"Submission for that object failed on MAIN server")
        return False, submission_exists
    ...
    if os.path.exists(report_file):
        ...
        if mode == "submit":
            ...
            if "Submission(s) validated successfully." in report_text:
                print("Submission validation succeeded")
                return True, submission_exists
            else:
                ...
                if not os.path.exists(submission_path):
                    ...
                    for item in WEBIN_SUBMISSION_RESULT_FILES:
                        if not os.path.exists(os.path.join(submission_path, item)):
                            print(f"No {item} found in {os.path.join(submission_path, item)}")
                            return False
                    print(f"Result folder {submission_path} is OK")
    return True
...
def main():
    ...
    run_webin_cli(manifest=manifest_for_submission, context=args.context, webin=webin, password=password, mode=args.mode, test=args.test)
    ...
    if result_status:
        print(f"Submission/validation done for {manifest_for_submission}")
    else:
        print(f"Submission/validation failed for {manifest_for_submission}")
